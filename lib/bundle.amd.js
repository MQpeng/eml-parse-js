	...
	function read(eml, options, callback) {
	    ...
	    function _append(headers, content, result) {
	        ...
	        if (!result.html && contentType && contentType.indexOf('text/html') >= 0) {
	            ...
	            try {
	                if (encoding === 'base64') {
	                    result.html = jsBase64.Base64.decode(result.html);
	                }
	                else if (jsBase64.Base64.btoa(jsBase64.Base64.atob(result.html)) == result.html) {
	                    result.html = jsBase64.Base64.atob(result.html);
	                }
	            }
	            catch (error) {
	                console.error(error);
